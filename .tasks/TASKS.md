...
- [x] **Publish v1.3.5 updater result panel** - made the retained tray-helper close instruction unmistakable with a color-independent ASCII result panel and published/installed the complete immutable patch (done 2026-07-22) (owner codex) #r135
  ...
  - [x] Publish immutable v1.3.5 and pass all fresh-public-byte plus production-site checks.
  - [x] Install official v1.3.5 and physically verify the tray completion panel and runtime/receipt behavior.
...
- [x] **Publish v1.3.0 detached Windows app-start** - made every Windows start surface hand off to the branded GUI launcher and console-free runtime, then closed the complete immutable release gate (done 2026-07-19) (owner codex) #wstart
  ...
  - [x] Add unit, packaging, x64/ARM64, and disposable lifecycle proof for terminal-independent runtime ownership.
  - [x] Record ADR 0036 and synchronize current guidance plus both changelogs.
  - [x] Pass the exact v1.3.0 candidate across every platform producer and final assembly.
  - [x] Fast-forward the unchanged candidate SHA to main and pass ordinary same-SHA CI.
  - [x] Publish immutable v1.3.0 and pass all eight fresh-public-byte jobs plus live-site verification.
  - [x] Close readiness and board evidence, then remove the merged temporary release branch.
