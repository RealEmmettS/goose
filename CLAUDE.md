...
The repo holds the brief, the original app's files as reference, and two planning docs.

## Read these first (source-of-truth pointers)

- `claude_plan.md` — **the authoritative implementation plan**: architecture, build
  milestones M0–M17, packaging pipeline, locked decisions, and a ranked risk table. Start here.
- `codex_plan.md` — a parallel plan written by Codex. Cross-reference it, but `claude_plan.md`
  is canonical for this assistant.
...
- **Planned Rust layout (plan §5): a Cargo workspace** — a platform-agnostic `honk-engine`
