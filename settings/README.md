...
The pinned AccessKit bridge forwards the existing semantics to Windows UI Automation and Linux AT-SPI (ADR 0042); macOS keeps the SDK's native implementation. On Windows, keep `honk_settings_accessibility.dll` beside the executable. The release build helper records and verifies both payload identities and includes dependency notices. Native Windows and hosted GTK checks exercise names, editor focus, input, modal isolation and isolated saves; the first published qualification is recorded in [the settings release evidence](../docs/readiness/v1.4.0-readiness.md). Every release repeats native architecture and packaging checks.

Platform & status reports fullscreen observation and do-not-disturb availability separately. These describe the running backend's observations, independently of the saved manners preferences. A stopped or older runtime reports unprobed observations; refreshing status preserves unsaved edits. Mac fullscreen uses existing Accessibility permission, while Mac and Linux DND remain unsupported until their own native signal is qualified (ADR 0049).
