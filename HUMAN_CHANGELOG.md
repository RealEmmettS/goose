...
### Fixed
- Recognize when GNOME observations are turned off at the same moment Goose checks them, so it can show the correct status while keeping observations off.
- Show the Goose artwork for the running Linux controls as well as their launcher, without adding a duplicate application entry. Keep that integration consistent when an installation is restored or removed.
...
### Behind the scenes
- Read or save the complete installer agreement from the website footer, including the real license and the entire Great Honk Accord. Keep the wording identical to the installer.
...
### Behind the scenes
- Include the corrected GNOME disconnect status and repeat the complete desktop and download checks before publishing the final manners update.
- Keep private drag tests clear of the goose's walk back and record which window actually receives the pointer, while preserving the same required desktop behavior.
