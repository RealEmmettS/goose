use super::{directory, hyprland_consent, Error};
use honk_control::{CapabilityStatus, WaylandStatus};
use honk_platform_linux::hyprland::{Frame, Observer};
use std::time::{Duration, Instant};

#[derive(Default)]
pub(crate) struct HyprlandRuntime {
    observer: Option<Observer>,
    consent: Option<hyprland_consent::Consent>,
    last_check: Option<Instant>,
    failed: bool,
    observed: bool,
}

impl HyprlandRuntime {
    pub(crate) fn start() -> Self {
        let mut runtime = Self::default();
        match directory().and_then(|path| hyprland_consent::read(&path).map_err(Into::into)) {
            Ok(None) => {}
            Ok(Some(_)) => {
                if let Err(error) = runtime.enable() {
                    eprintln!("honk300: optional Hyprland observations unavailable ({error})");
                }
            }
    ...
    pub(crate) fn enable(&mut self) -> Result<(), Error> {
        let desired = (|| -> Result<_, Error> {
            let consent = hyprland_consent::read(&directory()?)?
                .ok_or("Use Hyprland setup before enabling observations")?;
            if !consent.current() {
                return Err("Repeat Hyprland setup for this observation update".into());
            }
            Ok(consent)
        })();
    ...
    pub(crate) fn poll(&mut self) -> Option<Frame> {
            ...
            .is_none_or(|at| at.elapsed() >= Duration::from_millis(100))
        {
            ...
            let saved = directory()
                .ok()
                .and_then(|path| hyprland_consent::read(&path).ok())
                .flatten();
