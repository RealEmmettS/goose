//! Separate, private consent for read-only Sway observations.
...
const RECORD: &str = "sway.json";
const BOUNDARY: &str = "sway-observe-1";
...
pub(super) fn read(directory: &Path) -> io::Result<Option<Consent>> {
    ...
    if bytes.len() > 1024 {
        return Err(invalid("Sway consent exceeds its bound"));
    }
    let consent: Consent =
        serde_json::from_slice(&bytes).map_err(|_| invalid("Invalid Sway consent record"))?;
    if consent.boundary.len() > 64
        || consent.nonce.len() != 32
        || !consent.nonce.bytes().all(|byte| byte.is_ascii_hexdigit())
    {
        return Err(invalid("Invalid Sway consent identity"));
    }
    Ok(Some(consent))
}
...
pub(super) fn install(directory: &Path, nonce: &str) -> io::Result<Consent> {
    if nonce.len() != 32 || !nonce.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err(invalid("Invalid Sway setup identity"));
    }
    let _lock = super::installed::lock_record(directory, ".sway-lock")?;
    ...
    let temporary = directory.join(format!(".sway-{nonce}.new"));
...
mod tests {
    use super::*;
    #[test]
    fn sway_revocation_preserves_kde_and_unknown_records() {
