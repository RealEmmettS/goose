...
## Native accessibility finding and resolution

Windows computer use captured and operated the actual settings HWND, including saving an isolated draft and reading back its file. The original Windows UI Automation tree exposed only the canvas pane and window chrome. SDK 0.5.4 did not wire its widget-accessibility callback on Windows or GTK; the inspected 0.10.1 Windows package also lacked that callback. SDK automation semantics alone were not evidence of a Windows screen-reader bridge.

ADR 0042 resolves that finding through the bounded native AccessKit bridge. Actual Windows UIA and GNU Linux AT-SPI controls, modal focus, saved readback and the complete native package matrix pass in the final v1.4.0 qualification. The [readiness record](../readiness/v1.4.0-readiness.md) retains the exact source and all publication/public-download evidence. This establishes the native accessibility path; unavailable physical Mac/Pi equipment and the historical installed Windows elevation remain separate acceptance limits.
