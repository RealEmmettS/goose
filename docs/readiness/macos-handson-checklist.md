...
Use this runbook on a real Mac to qualify the exact v0.3.3 release candidate and then verify the
published artifacts. It supplies the native evidence required by `#m16r` and the macOS portions
of `#m20q`; `docs/readiness/v0.3.3-readiness.md` remains the release source of truth. ADR 0020
...
- Start from the full 40-character candidate commit selected for v0.3.3. Candidate mode must
...
- `~/Library/Application Support/honk300/install-receipt.json` uses schema
  `honk300.install.v1`, channel `dmg`, layout `mac-app`, and the exact v0.3.3 tag and full
  candidate commit;
...
scoped opt-ins: they remove only the v0.3.3 Honk300 prompt marker and reset only bundle id
...
- a receipted published v0.3.2 app updates to v0.3.3 through `honk300 update`;
...
2. create and push the single immutable `v0.3.3` tag;
...
notarization, stapling, Gatekeeper, DMG layout, graphical install, and the live v0.3.2-to-v0.3.3
...
secondary terminal install. Verify the production macOS link returns the real immutable v0.3.3
DMG and that release-manifest validation fails closed if it is absent.

## 10. Record and close

Append a dated evidence entry to `docs/readiness/v0.3.3-readiness.md` and the granted native
