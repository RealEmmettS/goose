...
| R06 | Medium | Linux collect support is disabled, so note/meme commands cannot provide the Windows/macOS experience. | Owned companion props pass native production GNU/musl x64/ARM64 host/runtime checks (34189363866) and labwc (34186038608); positioning remains separate from compositor placement. | Corrected; second-stage publication pending |
| R07 | Medium | Config editors load a whole model and save later without detecting intervening edits. Adding a GUI makes concurrent edits more likely. | GUI/TUI share revision-aware atomic persistence and a cooperating-writer lock. Conflict, compatible unknown-key, comment, validation and exact-runtime reload regressions pass; isolated native Windows saves read back correctly. | Corrected; release qualification pending |
| R08 | Low | install.rs combines provenance, user-media migration, autostart, platform mutation, and tests in roughly 245 KB. Current guidance duplicates years of release history. | Extracted provenance, autostart, mutable-media migration and existing tests; public lifecycle API retained. Active guidance points to complete historical copies. Full Windows-host workspace regressions, strict clippy and 129 Python checks pass; native cfg builds remain part of CI. | Corrected; platform checks pending |
| R09 | Medium | Existing ARM64 builds are not Pi desktop acceptance. Native SDK's Linux native-only host requires GTK4. | Exact production companions pass GNU GTK 4.6 and native Alpine checks; ARM64 labwc 0.7.1 scales one/two pass and captures were inspected. Experimental Pi guidance records installation and remaining hardware acceptance. | Automated qualification passed; physical Pi open |
...
- The actual Windows and GNU Linux accessibility checks exposed a 64-widget truncation after a page gained changed-state labels, hiding Save from assistive readers. Both toolkit and bridge now retain up to 256 widgets. The isolated native Windows save and saved-status readback passed after correction; hosted OS-pattern checks are rerunning.
...
- Native Mac compilation detected a misplaced cfg attribute during installer module extraction. Restored the exact original function-level cfg boundaries and limited the root autostart import to its actual callers. Windows strict clippy passes; final Mac compilation remains a gate.
