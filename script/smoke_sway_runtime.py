"""Actual Rust owner, native settings and live Sway observation lifecycle."""
import json
import os
...
'''
    ...
    record = Path(os.environ['XDG_DATA_HOME']) / 'honk300/wayland/sway.json'
    runtime = ui = None
    logs = []
    states = []
    ...
    def status():
        ...
        return json.loads(control('integrations', 'sway', 'status'))
    ...
    def workers():
        return [p.read_text().strip() for p in Path(f'/proc/{runtime.pid}/task').glob('*/comm')
                if p.read_text().strip() == 'sway-observer']
    ...
    try:
        ...
        invoke('Set up Sway')
        wait(lambda: find('Enable Sway observations', Atspi.Role.DIALOG), 'native Sway consent dialog')
        invoke('Cancel')
        ...
        trace(True, True)
        window.unfullscreen()
        wait(lambda: find_window('Honk300 ordinary Sway probe')['fullscreen_mode'] == 0, 'fixture leaves fullscreen')
        trace(False, False)
        invoke('Refresh status')
        invoke('Appearance')
        wait(lambda: (node := find('Reduced motion', Atspi.Role.TOGGLE_BUTTON)) and
             node.get_state_set().contains(Atspi.StateType.PRESSED), 'draft survives Sway setup and status')
        assert config.read_text() == original
        invoke('Platform & status')
        invoke('Remove Sway observations')
        ...
        assert runtime.returncode == 0
        runtime = None
        control('integrations', 'sway', 'remove')
        assert not record.exists() and not status()['installed']
