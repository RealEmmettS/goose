...
PE_SUBSYSTEMS = {2: "windows-gui", 3: "windows-console"}
...
def read_pe_subsystem(data: bytes) -> int:
    if len(data) < 0x40 or data[:2] != b"MZ":
        raise ValueError("not a PE binary (missing DOS header)")
    pe_offset = struct.unpack_from("<I", data, 0x3C)[0]
    optional_header = pe_offset + 24
    subsystem_offset = optional_header + 68
    if subsystem_offset + 2 > len(data) or data[pe_offset : pe_offset + 4] != b"PE\0\0":
        raise ValueError("not a PE binary (missing optional-header subsystem)")
...
def main() -> int:
    ...
    parser.add_argument("--subsystem", type=parse_machine)
    ...
    if args.subsystem is not None:
        if args.format != "pe":
            parser.error("--subsystem is valid only with --format pe")
        try:
            actual_subsystem = read_pe_subsystem(data)
        except ValueError as error:
            parser.error(str(error))
        if actual_subsystem != args.subsystem:
            parser.error(
                f"PE subsystem mismatch: expected {args.subsystem:#x} "
                f"({PE_SUBSYSTEMS.get(args.subsystem, 'unknown')}), got "
                f"{actual_subsystem:#x} ({PE_SUBSYSTEMS.get(actual_subsystem, 'unknown')})"
            )
        subsystem_text = (
            f", subsystem {actual_subsystem:#x} "
            f"({PE_SUBSYSTEMS.get(actual_subsystem, 'unknown')})"
        )
    label = (ELF_MACHINES if args.format == "elf" else PE_MACHINES).get(actual, "unknown")
    print(
        f"{args.binary}: {args.format.upper()} machine {actual:#x} ({label})"
        f"{subsystem_text}"
    )
    return 0
